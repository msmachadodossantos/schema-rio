 ...
 */
function getAllInputTextElements(): NodeListOf<HTMLInputElement> {
 ...
 * @returns {void}
 */
function updatePlaceholders(inputElements: NodeListOf<HTMLInputElement>): void {
  inputElements.forEach((inputElement: HTMLInputElement) => {
    const dataAttribute = inputElement.getAttribute('data-schema-rio');
...
function handleInput(nodes: NodeListOf<HTMLInputElement>): void {
  ...
  nodes.forEach((input) => {
    ...
    input.addEventListener('input', (event) => {
      ...
      const patternIndex = inputValue.length % pattern.length;
      ...
      const inputChar = inputValue.charAt(inputValue.length - 1);
 ...
 * @returns {void}
 */
function runSchemario(): void {
  const inputElements = getAllInputTextElements();
